...
`,
  ...
  it('works with entry descriptor', () => {
    setupWebpackProject({
      'webpack.config.js': `
const Plugin = require(${JSON.stringify(rootPath)});
module.exports = {
...
`,
      ...
      expect(stderr).toIncludeMultiple([
        'Error',
        'Some error happened',
        `${path.normalize('src/throw.js')}:4`,
        `${path.normalize('src/index.js')}:3`,
      ]);
    });
  }
...
`,
  ...
  it('works with differently cased files', () => {
    setupWebpackProject({
      'webpack.config.js': `
const Plugin = require(${JSON.stringify(rootPath)});
module.exports = {
...
`,
      'src/withFilesInSameDir.js': `
import { greeting1, greeting2 } from './c1';
...
`,
      'src/c1/greeting.js': `
export const greeting1 = 'Hi, there!';
export const greeting2 = 'Hey, dude!';
`,
      'src/withFilesInDifferentDirs.js': `
import { greeting1 } from './c2-1';
import { greeting2 } from './c2-2';
console.log(greeting1, greeting2);
`,
      'src/c2-1/index.js': `
export * from './greeting';
`,
      'src/c2-1/greeting.js': `
export const greeting1 = 'Hi, there!';
`,
      'src/c2-2/index.js': `
export * from './GREETING';
`,
      'src/c2-2/GREETING.js': `
export const greeting2 = 'Hey, dude!';
`,
    });

    subCaseWithFilesInSameDir();
    subCaseWithFilesInDifferentDirs();

    function subCaseWithFilesInSameDir() {
      try {
        fs.rmSync('dist', { recursive: true });
      } catch {}
      useSubEntry('withFilesInSameDir');
      const { stdout: webpackStdout, status: webpackStatus } = execWebpack();
      expect(webpackStatus).toBe(0);
      expectCommonDirToIncludeAllFiles([
        'dist/index.js',
        'dist/withFilesInSameDir.js',
        'dist/c1/index.js',
      ]);
      expect(webpackStdout).toIncludeMultiple(['WARNING in external', 'differ in casing']);
      const { stdout: nodeStdout, status: nodeStatus } = execNode('dist/index.js');
      expect(nodeStatus).toBe(0);
      expect(nodeStdout).toIncludeMultiple(['Hi, there!', 'Hey, dude!']);
    }

    function subCaseWithFilesInDifferentDirs() {
      try {
        fs.rmSync('dist', { recursive: true });
      } catch {}
      useSubEntry('withFilesInDifferentDirs');
      const { stdout: webpackStdout, status: webpackStatus } = execWebpack();
      expect(webpackStatus).toBe(0);
      expectCommonDirToIncludeSameFiles([
        'dist/index.js',
        'dist/withFilesInDifferentDirs.js',
        'dist/c2-1/index.js',
        'dist/c2-1/greeting.js',
        'dist/c2-2/index.js',
        'dist/c2-2/GREETING.js',
      ]);
      expect(webpackStdout).not.toIncludeMultiple(['WARNING in external', 'differ in casing']);
      const { stdout: nodeStdout, status: nodeStatus } = execNode('dist/index.js');
      expect(nodeStatus).toBe(0);
      expect(nodeStdout).toIncludeMultiple(['Hi, there!', 'Hey, dude!']);
    }
  ...
  if (os.platform() === 'win32') {
    it('generates require paths in posix format even on win32', () => {
      setupWebpackProject({
        'webpack.config.js': `
const Plugin = require(${JSON.stringify(rootPath)});
module.exports = {
...
`,
      ...
      expect(execWebpack().status).toBe(0);
      expectCommonDirToIncludeAllFilesAnd({
        'dist/index.js': (t) => expect(t).toInclude('require("./constants.js")'),
      });
    });
  }
});
